  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2">
        <img src={heroImage} alt="CosyHome Conciergerie" className="w-full h-full object-cover" />
      ...
      <div className="flex-1 flex items-center justify-center p-8 md:p-16 bg-background">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-primary text-primary-foreground px-4 py-2.5 text-sm font-medium hover:bg-primary/90 transition-colors"
            >
              {t.submit}
            </button>
          </form>

          <p className="text-small text-muted-foreground mt-8 text-center">{t.noCode}</p>
        </div>
      </div>
    </div>
  )
